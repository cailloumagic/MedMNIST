...
for predicted in prediction:
    ...
    for augmented in augmentation:
        ...
        for sev in range(1, 6):
            ...
            if predicted == True:
                predict = "_correctly-predicted"
            else:
                predict = "_incorrectly-predicted"
            ...
            for root, dirs, files in os.walk(directory_path_0):
                for file in files:
                    if file.endswith("_rmses"):
                        ...
                        elif not augmented and predicted and f"sev{sev}" in file and "augmented" not in file and not "false" in file:
                            if parent_dir_plot not in parent_dirs_with_csv_plot:
                                parent_dirs_with_csv_plot.append(parent_dir_plot)
                            if parent_dir not in parent_dirs_with_csv and sev == 1:
                                parent_dirs_with_csv.append(parent_dir)

                        elif augmented and not predicted and f"sev{sev}" in file and "augmented" in file and "false" in file:
                            if parent_dir_plot not in parent_dirs_with_csv_plot:
                                parent_dirs_with_csv_plot.append(parent_dir_plot)
                            if parent_dir not in parent_dirs_with_csv and sev == 1:
                                parent_dirs_with_csv.append(parent_dir)

                        elif not augmented and not predicted and f"sev{sev}" in file and "augmented" not in file and "false" in file:
                            if parent_dir_plot not in parent_dirs_with_csv_plot:
                                parent_dirs_with_csv_plot.append(parent_dir_plot)
                            if parent_dir not in parent_dirs_with_csv and sev == 1:
                                parent_dirs_with_csv.append(parent_dir)
                    ...
                    for k in range(class_nb):  # classes
                        ...
                        for i in range(min_range, max_range):  # perturbations
                            ...
                            if dir_index == 0:
                                ...
                                axs[dir_index, i - min_range].set_title(f"Deltas AUC: {auc_data_sev[i - min_range]}",
                                                                        fontsize=16)
            ...
            save_path = os.path.join(directory_path_1, f'RMSEs_plot_combined_sev{sev}{augment}{predict}.png')
        ...
        for dir_index, parent_dir in enumerate(parent_dirs_with_csv):  # Datasets
            ...
            save_path = os.path.join(directory_path_1,
                                     f'AUC_scatterplot_{os.path.basename(os.path.dirname(parent_dir))}{augment}{predict}.png')
            plt.savefig(save_path)
            plt.close(fig)
        ...
        for dir_index, parent_dir in enumerate(parent_dirs_with_csv):  # Datasets
            ...
            for directory in os.listdir(parent_dir):
                class_dir_path = os.path.join(parent_dir, directory)
                if directory.startswith("class") and os.path.isdir(class_dir_path) and os.listdir(
                        class_dir_path):
                    class_labels.append(directory.replace("class_", "class: "))
                    ...
                    for j in range(8):  # layers
                        ...
                        for h in range(5):  # perturbations
                            ...
                            for i in range(min_range, max_range):  # severities
                                ...
                                axs[h, j].scatter(x_data, y_data, marker=marker_label[l],
                                                  color=color_label[i - min_range])
                            ...
                            if k == 0:
                                # Add text to the left of the first plot of each row
                                axs[h, 0].text(-0.3, 0.5, title_label[h],
                                               va='center', ha='right', rotation=90,
            ...
            severities_legend_handles = [
                Patch(facecolor=color_label[h], edgecolor=color_label[h], label=f"Severity {h + 1}")
                for h in range(5)
            ]
            ...
            print("Saving scatter plot 2...")
            # Save the combined plot
            save_path = os.path.join(directory_path_1,
                                     f'AUC_scatterplot_2_{os.path.basename(os.path.dirname(parent_dir))}{augment}{predict}.png')
            plt.savefig(save_path)
            plt.close(fig)
        ...
        for dir_index, parent_dir in enumerate(parent_dirs_with_csv):  # Datasets
            ...
            save_path = os.path.join(directory_path_1,
                                     f'AUC_boxplot_{os.path.basename(os.path.dirname(parent_dir))}{augment}{predict}.png')
            plt.savefig(save_path)
            plt.close(fig)
        ...
        for dir_index, parent_dir in enumerate(parent_dirs_with_csv):  # Datasets
                ...
                for j in range(8):  # layers
                    for i in range(5):  # perturbations
                        ...
                        for k in range(5):  # severities
                            ...
                            box_color = color_label[k % len(color_label)]  # Using modulo to cycle through colors
                            hatch_pattern = hatch_label[
                                l % len(hatch_label)]  # Using modulo to cycle through hatch patterns
                            ...
                            axs[i, 0].text(-0.3, 0.5, title_label[i],
                                           va='center', ha='right', rotation=90,
            ...
            save_path = os.path.join(directory_path_1,
                                     f'AUC_boxplot_2_{os.path.basename(os.path.dirname(parent_dir))}{augment}{predict}.png')
            plt.savefig(save_path)
            plt.close(fig)
            print(f"            Box plot 2 {os.path.basename(os.path.dirname(parent_dir))} generated")
